...
The repository is one root Next.js application by default. Multi-tenant mode
adds independent applications under `apps/`, sharing the route files at that
directory's root and reusable npm workspace packages under `packages/`.
`tenants.config.mjs` names them, and `APP_VARIANT` selects exactly one for
development, type checking, building, serving, previewing, or deployment.
...
workspace packages. `APP_VARIANT` is a build and deployment selector, not a
user-role or authorization mechanism. See
