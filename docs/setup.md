...
| --------------------------------- | ----------- | ----------------------------------------------- |
| `APP_VARIANT`                     | Build/CLI   | Selects a registered app; defaults to `default` |
...
browser bundles. `APP_VARIANT` is consumed only by the repository command runner;
application code should not use it for authorization or runtime role selection.
Do not add `import.meta.env`, Vite prefixes, React Router loaders, or Vite plugins.
...
| Area                   | Required setup                                                                                                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Application topology   | Keep the root single app unless independent deployments need overlapping route trees; if so, follow the multi-app workspace guide and register every target. |
